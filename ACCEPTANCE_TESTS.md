...
| # | Acceptance criterion | Status | Evidence or remaining check |
| ---: | --- | --- | --- |
| 1 | Android text share stages Add and requires explicit confirmation | Automated source pass; device/manual remains | `ACTION_SEND text/*` routes to `android-share`; the view model explicitly disables auto-submit and displays Add before creation. |
| 2 | External `magnet:` deep link stages Add and requires explicit confirmation | Automated source pass; device/manual remains | `deep-link` is an explicit no-auto-submit source; live Android launch remains. |
...
| 5 | Shared/opened `.torrent` stages Add and requires explicit confirmation | Automated source pass; provider/manual remains | URI ingestion never submits; it parses and waits for **Add to TorBox** regardless of Share, Open with, or picker source. |
...
| 13 | Multi-file Share chooses a file and opens Android share sheet | Requires real TorBox account/device/manual | The selected-file flow now calls `requestdl` directly, visibly reports link preparation, closes the file sheet, and emits the Android share intent. The request contract is automated; the live chooser flow remains. |
